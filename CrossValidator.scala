...
class CrossValidator @Since("1.2.0") (@Since("1.4.0") override val uid: String)
  ...
  def askSuggestion(estimator: Estimator[_])= {
    ...
    val suggestion_response = parse(Http(suggestion_url).postData("").headers(Seq("content-type" -> "application/json", "connection" -> "close")).timeout(connTimeoutMs = 5000, readTimeoutMs = 5000).auth(this.token +":","").asString.body)
  ...
  def observeSuggestion(est: Estimator[_], metric: Double)= {
    ...
    (Http(observation_url).postData(swrite(Observe(this.suggestion_id, metric))).auth(this.token, "").timeout(connTimeoutMs = 5000, readTimeoutMs = 5000).headers(Seq("content-type" -> "application/json", "connection" -> "close"))).asString.body
  ...
  def SigFit(dataset: Dataset[_]): CrossValidatorModel = {
      ...
      val accMetrics = sc.accumulableCollection[mutable.ArrayBuffer[Double], Double](new mutable.ArrayBuffer[Double]())
      var metrics = new Array[Double](numModels)      
      var i = 0
      ...
      while(i <= numModels){
        var models = est.fit(trainingDataset, epm(0))
        accMetrics += eval.evaluate((models.asInstanceOf[Model[_]]).transform(validationDataset, epm(0)))
        var avg_metric: Double = (accMetrics.value).sum / (accMetrics.value).length
        print(avg_metric)
        accMetrics.value.clear
        this.observeSuggestion(est, avg_metric)
        print(this.suggestion_id)
        i += 1
        }
      }
